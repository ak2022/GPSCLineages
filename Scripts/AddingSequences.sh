...
paste <(ls "$MYPATH/$2" | sed 's/[.].*$//g') <(ls -d "./NewSeqs"/* | xargs realpath) > "./NewSkaInput.tsv"
...
bsub -w "done(BUILD)" -J "MERGE" -M8000 -n16 -R "select[mem>8000] rusage[mem=8000]" -R "span[hosts=1]" -o ./Outputs/mergeo.%J -e ./Outputs/mergee.%J 'ska merge -o all_samples.skf ./*.skf'
